...
if(form){
  ...
  function deliveryFee(){
    ...
    if(choice==="Delivery within 25 miles")return 25;
    if(choice==="Delivery over 25 miles")return 0;
    return 0;
  }
...
mailForm('#quote-form','Website Quote Request');
...
(() => {
  ...
  function priceForPreparation(preparation) {
    if (state.plan.weeklySpecial) return 10.99;
  ...
  function resetMealSelections() {
    Object.keys(state.meals).forEach(name => { state.meals[name] = 0; });
  ...
  function applyPlanMealVisibility() {
    const isWeekly = state.plan.weeklySpecial === true;

    mealCards.forEach(card => {
      const isWeeklyCard = card.dataset.meal === 'Weekly Special';
      card.hidden = isWeekly ? !isWeeklyCard : isWeeklyCard;
    });

    const customCard = section.querySelector('.custom-meal-card');
    if (customCard) customCard.hidden = isWeekly;

    const title = section.querySelector('#meal-step-title');
    const copy = section.querySelector('#meal-step-copy');

    if (title) {
      title.textContent = isWeekly
        ? 'Choose Your Weekly Special Quantity'
        : 'Select Your Bowls';
    }

    if (copy) {
      copy.textContent = isWeekly
        ? 'The Weekly Special is one chef-selected featured meal at $10.99 each. Use the + and − controls to choose how many you would like.'
        : 'Use the + and − controls to choose how many of each bowl you want. Your total must meet the minimum for the selected plan.';
    }
  }
  ...
  section.querySelectorAll('.package-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      state.plan = {
        ...
        weeklySpecial: card.dataset.weeklySpecial === 'true'
      };
      resetMealSelections();
      applyPlanMealVisibility();
      updateCart();
    });
  });
  ...
  zoneButton.addEventListener('click', () => {
    ...
    const withinRange = window.confirm(
      'Is this delivery address within 25 miles of ZIP code 32712?\n\nChoose OK if it is within 25 miles. Choose Cancel if it is farther away.'
    );

    if (!withinRange) {
      state.deliveryZip = '';
      state.deliveryFee = 0;
      result.textContent = 'This address is outside the 25-mile meal-prep delivery area. Please choose pickup.';
      updateCart();
      return;
    }
    ...
    result.textContent = `${zip}: $25 flat delivery fee within 25 miles of 32712.`;
    updateCart();
  });
  ...
  function openCustomModal() {
    if (state.plan.weeklySpecial) return;
  ...
  function renderReview() {
    ...
    section.querySelector('#checkout-review-details').innerHTML = `
      ...
      <div class="review-section">
        ...
        <div class="review-line"><span>Preparation</span><strong>${state.plan.weeklySpecial ? 'Chef-selected weekly feature' : state.plan.preparation}</strong></div>
